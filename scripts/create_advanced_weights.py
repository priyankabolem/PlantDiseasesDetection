...
def add_dark_spots(img, intensity=0.5):
    ...
    for _ in range(n_spots):
        ...
        # Add halo effect
        cv2.circle(img, (x, y), radius + 3, (img[y, x] * 0.7).tolist(), 2)
...
def add_rust_patches(img):
    ...
    for _ in range(n_patches):
        ...
        for c in range(3):
            img[y1:y2, x1:x2, c] = (
                img[y1:y2, x1:x2, c] * (1 - mask) + patch_resized[:, :, c] * mask
            )
...
def add_blight_patterns(img):
    ...
    for _ in range(n_areas):
        ...
        color = (0.3 + np.random.random() * 0.2, 0.2 + np.random.random() * 0.1, 0.1)
...
def add_bacterial_spots(img):
    ...
    for _ in range(n_spots):
        ...
        color = (img[y, x, 0] * 0.5, img[y, x, 1] * 0.4, img[y, x, 2] * 0.3)
        cv2.circle(img, (x, y), radius, color, -1)

        # Yellow halo
        cv2.circle(img, (x, y), radius + 2, (0.7, 0.7, 0.3), 1)
...
def add_random_disease(img):
    ...
    disease_type = np.random.choice(["spots", "patches", "discoloration"])
