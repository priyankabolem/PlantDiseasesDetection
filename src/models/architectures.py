...
class ModelBuilder:
    ...
    def __init__(
        self, num_classes: int, input_shape: Tuple[int, int, int] = (224, 224, 3)
    ):
    ...
    def build_model(
        ...
        logger.info(
            f"Built {architecture} model with {model.count_params():,} parameters"
        )
        return model

    def _build_resnet50(
        self, pretrained: bool, freeze_base: bool, dropout_rate: float
    ) -> tf.keras.Model:
    ...
    def unfreeze_layers(
        self, model: tf.keras.Model, num_layers: int = 20
    ) -> tf.keras.Model:
